...
export function createMixedPracticeApi({ service, readBody, sendJson, complete, isConfigured }) {
  return async function handleMixedPractice(req, res, url) {
    ...
    try {
      ...
      if (path === 'next') return sendJson(res, 200, service.next(profileId, sessionId));
      if (path === 'answer') return sendJson(res, 200, service.answer(profileId, sessionId, questionId, body.answerIndex));
      if (path === 'hint') return sendJson(res, 200, service.hint(profileId, sessionId, questionId));
      if (path === 'assisted') return sendJson(res, 200, service.tutorReceived(profileId, sessionId, questionId, true));

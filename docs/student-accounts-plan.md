...
Melody authorized direct student signup on September 15, 2026. Replit preview and deployment explicitly use `AUTH_ALLOW_SIGNUP=1`: select Create account, choose a username and password, and receive a new owned workspace. Later sign-ins return the same account and workspace, including saved progress, preferences, and explicit coach notes. Signup never claims a submitted legacy ID. Study features remain available when Canvas is disconnected.

Students with a prepared workspace can still open its private enrollment link or select Use a setup invitation. An enrollment token takes precedence over public signup, can claim only the workspace bound to it, and is consumed atomically with account, membership, and session creation. Invalid invitations return to invitation entry; creating a new workspace requires an explicit choice. Deployments that deliberately leave `AUTH_ALLOW_SIGNUP` unset remain invitation-only. Account-recovery and institution-approved Canvas OAuth still need implementation.
...
Configure `DATABASE_URL`, stable `SESSION_SECRET`, `AUTH_REQUIRED=1`, and `AUTH_ALLOW_SIGNUP=1` for the Replit workflow and deployment. The tracked deployment command explicitly enables both authentication and direct signup. Configuration/database failures must never downgrade to the unauthenticated local mode.
...
The sign-in screen includes Create account, which immediately shows username and password fields when public signup is enabled. Use a setup invitation opens optional invitation entry for a prepared workspace. The browser keeps an accepted invitation in this tab's session storage for up to 24 hours so unfinished setup survives reloads; the server still enforces the invitation's actual expiry and single use. A replacement link clears the previous invitation, and successful authentication or an explicit return to sign-in clears setup storage. If session storage is unavailable, the original fragment remains as the reload fallback. Account passwords are never placed in browser storage. Existing students use their selected username and password on later visits. There is no password-reset endpoint, recovery email, public invite-issuance endpoint, or administrative sharing UI yet.

Settings includes Account information and an optional Canvas connection card. A signed-in student can add or update their own school's token, see the verified school/user, disconnect, and retry incomplete saved-credential removal. The form clears entered tokens before waiting for a server response and never displays saved tokens. Lessons, practice, mastery checks, and saved learning progress remain available with Canvas disconnected. Remembered token updates await the configured database credential write before switching the local credential/session or reporting success. A rejected write returns an explicit save failure; credentials remain in the existing server-only file/database format, without application-level encryption. Database, file, and connection-mode writes are not one cross-storage transaction.
